...
def visualize_registration_colab(moving, fixed, affine_warped, warped, 
    ...
    and optional metrics display. Replaces the original visualize_registration_colab function.
    
    Args:
        moving: Moving/template images tensor [B, C, D, H, W]
        fixed: Fixed/target images tensor [B, C, D, H, W] 
        affine_warped: Images after affine transformation
    ...
    affine_idx = affine_warped.argmax(dim=1).cpu().numpy()
    ...
    for i in range(min(B, max_samples)):
        ...
        # Row 1: Transformation results
        axs[0,2].imshow(affine_idx[i, slice_idx], cmap='tab20', interpolation='nearest')
        axs[0,2].set_title(f'After Affine Registration\n{subject_id}', fontsize=14, fontweight='bold')
        axs[0,2].add_patch(Rectangle((2, 2), 20, 5, linewidth=2, edgecolor='green', facecolor='none'))
        axs[0,2].text(25, 7, 'Affine', fontsize=10, color='green', fontweight='bold')
        
        axs[0,3].imshow(warped_idx[i, slice_idx], cmap='tab20', interpolation='nearest')
        axs[0,3].set_title(f'After Non-Rigid Registration\n{subject_id}', fontsize=14, fontweight='bold')
        axs[0,3].add_patch(Rectangle((2, 2), 20, 5, linewidth=2, edgecolor='purple', facecolor='none'))
        axs[0,3].text(25, 7, 'Final', fontsize=10, color='purple', fontweight='bold')
        
        # Row 2: Difference maps and overlays
        # Compute difference maps
        moving_slice = moving_idx[i, slice_idx]
        fixed_slice = fixed_idx[i, slice_idx]
        affine_slice = affine_idx[i, slice_idx]
        ...
        diff_affine = np.abs(affine_slice.astype(float) - fixed_slice.astype(float))
        ...
        axs[1,2].imshow(diff_affine, cmap='hot', interpolation='nearest')
        axs[1,2].set_title('After Affine Difference\n(Affine vs Target)', fontsize=12)
        
        axs[1,3].imshow(diff_final, cmap='hot', interpolation='nearest')
        axs[1,3].set_title('Final Difference\n(Registered vs Target)', fontsize=12)
        ...
        if show_metrics:
            ...
            dice_affine = calculate_dice(affine_slice, fixed_slice)
...
Improvement: {dice_final - dice_initial:.3f}"""
        ...
        legend_elements = [
            ...
            mpatches.Patch(color='green', label='Affine Result'),
...
def visualize_training_progress(dice_scores, reg_losses, epochs, save_path=None):
    """
    Visualize training progress over epochs
    
    Args:
        dice_scores: List of dice scores per epoch
        reg_losses: Dictionary of regularization losses per epoch
    ...
    for loss_name, values in reg_losses.items():
        ax2.plot(epochs, values, '-o', label=loss_name, linewidth=2, markersize=4)
    
    ax2.set_xlabel('Epoch', fontsize=12)
    ax2.set_ylabel('Loss Value', fontsize=12)
    ax2.set_title('Regularization Losses Over Time', fontsize=14, fontweight='bold')
    ax2.legend()
...
visualize_registration_colab_enhanced(
    moving, fixed, affine_warped, warped,
