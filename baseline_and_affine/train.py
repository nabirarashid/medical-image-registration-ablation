...
def train(
    ...
    for batch_idx, (moving, fixed) in enumerate(
        ...
        print(
            f"Affine Dice Loss: {dice_loss_affine:.4f} -> Final Dice: {dice_loss_val:.4f}"
        )
        ...
        if batch_idx < 3:
            ...
            print(
                f"  Debug - Max deformation: {max_def:.6f}, Mean deformation: {mean_def:.6f}"
            )

        # Combined loss: main task losses only
        loss = composite_loss(warped_template, fixed)
...
def main():
    ...
    parser.add_argument(
        "--save_model_path",
        type=str,
        default="checkpoints/affine_unet_model.pth",
        help="Path to save the trained model",
    )

    # New hyperparameters for regularization - more lenient defaults
    parser.add_argument(
        "--displacement_weight",
        type=float,
        default=0.01,
        help="Weight for displacement magnitude loss",
    )
    parser.add_argument(
        "--rigidity_weight",
        type=float,
        default=0.005,
        help="Weight for local rigidity loss",
    )
    parser.add_argument(
        "--max_displacement",
        type=float,
        default=10.0,
        help="Maximum allowed displacement",
    )
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0:
            model_path = f"checkpoints/affine_unet_model_epoch_{epoch + 1}.pth"
            checkpoint = {
